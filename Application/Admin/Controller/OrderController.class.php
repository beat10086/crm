...
class  OrderController extends   BaseController  {
   ...
   public  function  getList () {
           if (IS_AJAX) {
               $this->ajaxReturn((new OrderModel())->getList(I('post.page'), I('post.rows'), I('post.order'),
   ...
   public  function  register (){
       if(IS_AJAX){
             $code=(new OrderModel())->register(I('post.title'), I('post.original'), I('post.cost'),
                                                I('post.documentary_id'), I('post.details'),
                                                I('post.contract'),I('post.product_outlib'));
             if($code>0){
